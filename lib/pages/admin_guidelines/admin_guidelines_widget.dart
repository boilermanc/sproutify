...
class AdminGuidelinesWidget extends StatefulWidget {
  const AdminGuidelinesWidget({super.key});

  static String routeName = 'AdminGuidelines';
  static String routePath = '/admin/guidelines';

  @override
  State<AdminGuidelinesWidget> createState() => _AdminGuidelinesWidgetState();
}

class _AdminGuidelinesWidgetState extends State<AdminGuidelinesWidget> {
  ...
  Future<void> _loadStatistics() async {
    try {
      ...
      final totalUsers = (totalUsersResponse as List).length;
      ...
      final acceptedCount = (acceptedResponse as List).length;
  ...
  Future<void> _loadUsersNotAccepted() async {
    try {
      // Get all users and filter for null acceptance in Dart
      // (Supabase Flutter doesn't have a direct .is() method for null checks)
      final allUsersResponse = await SupaFlow.client
          .from('profiles')
          .select('id, email, username, first_name, last_name, created_at, community_guidelines_accepted_at')
          .order('created_at', ascending: false)
          .limit(1000);

      final List<dynamic> allUsers = allUsersResponse as List<dynamic>;
      final List<dynamic> filteredUsers = allUsers
          .where((user) => (user as Map<String, dynamic>)['community_guidelines_accepted_at'] == null)
          .take(100)
          .toList();

      setState(() {
        _usersNotAccepted = filteredUsers
  ...
  Widget _buildUserCard(Map<String, dynamic> user, {required bool showReset}) {
    ...
    // Ensure displayName is not empty for avatar
    if (displayName.isEmpty) {
      displayName = 'U'; // Default to 'U' for User
    }
